...
// Fetch all character entries from the ems_application table
$sql = "SELECT * FROM ems_application";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'accept') {
        ...
        $stmt = $conn->prepare("UPDATE ems_application SET status = 'accepted' WHERE id = ?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
            // Also update user role to 'ems' in the users table using discord_id
            $stmt->close(); // Close the previous statement
            $stmt = $conn->prepare("UPDATE users SET role = 'ems' WHERE discord_id = ?");
            $stmt->bind_param("s", $discord_id);
            $stmt->execute();
            $stmt->close();

            // Send notification to user
            $notificationMessage = "Your application has been accepted into the EMS department. Congratulations!";
        ...
        $stmt = $conn->prepare("UPDATE ems_application SET status = 'rejected' WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();
    }
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['reason_to_join']); ?></td>
